...
- **`src/core/`** - 纯业务逻辑，无 I/O 依赖
  - `app.ts` - 编排所有操作（createBook、writeChapter、completeChapter、prepareSync 等）
  ...
  - `llm.ts` - OpenAI Responses API 客户端（流式 + 非流式）
...
- **使用 Responses API 而非 Chat API** - 使用 `client.responses.create()` 配合流式事件
